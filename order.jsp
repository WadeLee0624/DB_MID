...
//Step 4: 執行 SQL 指令, 若要操作記錄集, 需使用executeQuery, 才能傳回ResultSet	
           ...
           sql="SELECT PID , P_NAME from PRODUCT p WHERE (p.PID) IN (SELECT s.PID FROM SALE s WHERE s.SID ='"+store+"' ) AND p.CID = '"+otype+"'";
		   ResultSet rs=stmt.executeQuery(sql);
		   rs.last(); 		   
           int total_content= rs.getRow();
           
		   rs=stmt.executeQuery(sql);
		   out.print("<table border=\"1\" align=\"center\" width =\"38%\" ><form id=\"order_type\" method=\"post\" action=\"insertof.jsp\" align=\"center\">  ");
//  逐筆顯示, 直到沒有資料(最多還是5筆)
           int s = 0;
           while(rs.next())
                {                 					
                 out.println("<tr><td bgcolor=\"#FFFFFF\">&nbsp&nbsp&nbsp<input TYPE=\"checkbox\" name=\"dish\" VALUE=\""+rs.getString(1)+"\"  width=\"50%\">"+rs.getString(2)+"</td><td bgcolor=\"#FFFFFF\"  width=\"10%\"  >&nbsp&nbsp&nbsp數量<input type=\"text\" name=\"num\" size=\"4\"></td></tr>"); 
                }
		         out.println("<tr><td colspan=\"2\" align=\"center\" bgcolor=\"#FFFFFF\"><input type=\"submit\" value=\"新增\" ></td></tr></form></table>");
//Step 6: 關閉連線
          con.close();
      }
    }
